...
async function processTask(page: Page, task: any) {
  try {
    ...
    const targetButtons: ElementHandle[] = [];
    for (const b of rawButtons) {
      const txt = await page.evaluate(el => el.textContent, b);
      if (txt === '답글 달기' || txt === 'Reply') targetButtons.push(b);
    }
    ...
    for (const btn of targetButtons) {
      try {
        ...
        if (!commentInfo || !commentInfo.user || commentInfo.user.length < 2) continue;
        
        // 본인/답글 제외 로직
        if (commentInfo.msg?.includes(task.reply_content.slice(0, 3))) continue;
        ...
        if (!existing) {
          ...
          if (postBtn && (postBtn as ElementHandle).asElement()) {
            await (postBtn as ElementHandle).click();
...
async function sendDMWithFollow(page: Page, username: string, message: string) {
  try {
    ...
    if (followBtn && (followBtn as ElementHandle).asElement()) {
      console.log('팔로우 버튼 발견, 클릭 중...');
      await (followBtn as ElementHandle).click();
    ...
    if (messageBtn && (messageBtn as ElementHandle).asElement()) {
      console.log('메시지 창으로 이동...');
      await (messageBtn as ElementHandle).click();
