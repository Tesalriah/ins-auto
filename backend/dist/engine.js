...
async function processTask(page, task) {
    try {
        ...
        const myUsername = await page.evaluate(() => {
            ...
            const profileLink = document.querySelector('a[href^="/"][role="link"] img[alt*="프로필 사진"]')?.closest('a');
        ...
        for (const target of targets) {
            try {
                ...
                const posted = await page.evaluate(() => {
                    ...
                    if (postSpan) {
                        const btn = postSpan.closest('div[role="button"], button');
                        if (btn) {
                            btn.click();
                            return true;
                        }
...
async function sendDMInNewTab(browser, username, message) {
