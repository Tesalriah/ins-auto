...
- **유저네임 필터링 (Enhanced)**: 시간 정보(`10분`, `1시간`, `1일`, `212주` 등)가 담긴 링크를 작성자 아이디로 오판하지 않도록 '분', '시간', '주', '일' 키워드 필터링 적용됨.
...
- **실행 속도 최적화 (Aggressive)**: 모든 `setTimeout`과 타이핑 지연(`30ms`)을 기능 가능한 최소치로 단축함. 페이지 로드 대기를 `domcontentloaded`로 설정하여 반응 속도를 극대화함.
